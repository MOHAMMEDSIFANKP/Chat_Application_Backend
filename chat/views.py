    ...
    def get(self, request, id):
        ...
        try:
            ...
            if selectoption == 'People':
                ...
                friends_ids = FriendsList.objects.filter(friends_id=id).values_list('user_id', flat=True)
                requests_users = User.objects.filter(id__in=friends_ids)
                ...
                myfriends_instance = user_instance.friends_list.all()
...
class RemoveFriendsView(APIView):
    def post(self, request):
        ...
        # Remove the friendship from the user's friend list
        user_instance = User.objects.get(id=user_id)
        user_instance.friends_list.remove(friends_id)
        
        # Remove the friendship from the friend's friend list
